...
class CelebAHQ(data.Dataset):
    ...
    * **Partition mode** (``use_facial_identity=False``):
    ...
    def __init__(self,
        ...
        # Inversion paths
        self.inv_data_dir  = osp.join('datasets', 'inv', dataset_name, 'data')
        self.inv_codes_dir = osp.join('datasets', 'inv', dataset_name, 'latent_codes')
        ...
        if fake_nn_map is not None:
            ...
            print("Fake-NN map Loading... Complete! ")
        else:
        ...
        if self.use_facial_identity:
            ...
            raise Exception("Partioned data Not to be used in FaceAnonyMixer")
    ...
    def _prepare_identity_based_data(self):
        identity_dir = osp.join(self.root_dir, 'images')
    ...
    def __getitem__(self, idx):
        ...
        if self.fake_nn_map is not None:
            img_basekey = osp.join(osp.basename(osp.dirname(img_path)), osp.basename(img_path)) # ID/image_number.png
            nn_key = self.nn_map_dict.get(img_basekey, '')
            if nn_key:
