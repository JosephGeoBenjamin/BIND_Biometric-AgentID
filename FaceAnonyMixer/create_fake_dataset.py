...
def main():
    ...
    for i in tqdm(range(args.num_samples), desc='Generating'):
        ...
        mapping_out = G.mapping(z, label=None)
        w  = mapping_out['w']                                      # [B, 512]
        wp = G.truncation(w, trunc_psi=args.truncation)            # [B, num_layers, 512]
        #---
        ...
        for name, module in G.synthesis.named_modules():
            if hasattr(module, 'style') and hasattr(module, 'w_space_dim'):
                if layer_idx >= wp.shape[1]:   # safety cap
                    break
                styles_dict[name] = module.style(wp[:, layer_idx])
                layer_idx += 1
        #---
        ...
        with torch.no_grad():
            # img = G(styles_dict)
            synthesis_out = G.synthesis(wp)
            img           = synthesis_out['image']
