...
def _build_identity_mapping(dataset_root: str) -> dict:
    ...
    train_dir  = osp.join(dataset_root, 'images')
    id_mapping = {}
...
def _make_output_dirs(out_dir: str, dataset_root: str):
    ...
    train_dir    = osp.join(dataset_root, 'images')
    out_data_dir = osp.join(out_dir, 'images')
    out_code_dir = osp.join(out_dir, 'latent_codes')
    out_ref_dir  = osp.join(out_dir, 'reference')
    for folder in os.listdir(train_dir):
        if osp.isdir(osp.join(train_dir, folder)):
            ...
            os.makedirs(osp.join(out_ref_dir, folder), exist_ok=True)
    return out_data_dir, out_code_dir, out_ref_dir
...
def main():
    ...
    out_dir        = f"datasets/anonymised/{args.dataset}/"
    ...
    out_data_dir, out_code_dir, out_ref_dir  = _make_output_dirs(out_dir, dataset_root)
    ...
    # for data_ in tqdm(loader, desc="Grouping..."):
    #     img_id  = int(osp.basename(data_[2][0]).split('.')[0])
    #     id_fold = id_mapping.get(img_id)
    #     if id_fold:
    #         identity_groups[id_fold].append(data_)

    ## Lazy !!! --- Don't have TBs of RAM
    identity_groups = defaultdict(list)
    for idx in tqdm(range(len(dataset)), desc="Grouping..."):
        data_ = dataset.__getitem__(idx)
        image_path = data_[2]
        img_id  = int(osp.basename(image_path).split('.')[0])
        id_fold = id_mapping.get(img_id)
        if id_fold:
            identity_groups[id_fold].append(idx)

    print("Identity Grouping ....Complete!")

    # ── per-identity optimisation ─────────────────────────────────────────────
    desc = f'Anonymizing {args.dataset}' if args.verbose else ''
    for identity_folder, identity_idx in tqdm(identity_groups.items(), desc=desc):

        identity_data = [ dataset.__getitem__(idx) for idx in identity_idx ]
        identity_data = [(data_[0].unsqueeze(0), data_[1].unsqueeze(0), [data_[2]],
                        data_[3].unsqueeze(0), data_[4].unsqueeze(0),
                        data_[5].unsqueeze(0), data_[6].unsqueeze(0),
                        data_[7].unsqueeze(0), data_[8].unsqueeze(0),
                        [data_[9]])
        ...
        for lc, data_ in zip(latent_codes, data_refs):
            ...
            ref_path = osp.join(out_ref_dir, identity_folder, f'{img_id}_orig.jpg')
            tensor2image(data_[0].cpu(), adaptive=True).save(
                ref_path, 'JPEG', quality=75, subsampling=0, progressive=True,
            )

            ref_path = osp.join(out_ref_dir, identity_folder, f'{img_id}_recon.jpg')
            tensor2image(data_[5].cpu(), adaptive=True).save(
                ref_path, 'JPEG', quality=75, subsampling=0, progressive=True,
            )

            ref_path = osp.join(out_ref_dir, identity_folder, f'{img_id}_key_{nn_key[:6]}.jpg')
            tensor2image(data_[3].cpu(), adaptive=True).save(
                ref_path, 'JPEG', quality=75, subsampling=0, progressive=True,
            )
