...
def main():
    ...
    real_images_dir = osp.join(real_root, 'images')

    if not osp.isdir(real_images_dir):
        raise NotADirectoryError(
            f'Real dataset images directory not found: {real_images_dir}\n'
    ...
    real_ids = sorted(
        d for d in os.listdir(real_images_dir)
        if osp.isdir(osp.join(real_images_dir, d))
    )
    ...
    for real_id, fake_id in id_map.items():
        id_folder = osp.join(real_images_dir, real_id)
        # fake_img  = osp.join(fake_id, 'image.jpg')   # relative to fake_dataset_root
        fake_img  = fake_id   # relative to fake_dataset_root
